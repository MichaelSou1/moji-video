...
# 附录 E：step-150（终版）mini8 评测 + 训练收官（2026-08-05T03:25Z）

**训练已于 02:30Z 完整跑完 150/150 步**（34h15m，全程 0 OOM / 0 Traceback）。终版 ckpt `global_step_150` 已推 [`michaelsou/wan22-ti2v-5b-dancegrpo-ocr-step150`](https://modelscope.cn/models/michaelsou/wan22-ti2v-5b-dancegrpo-ocr-step150)。mini8 评测 24 min，0 Traceback（一次引擎显存自检失败后以 rollout 0.4/reward 0.35 利用率重启成功，纯运行参数）。

## E.1 七点全景

| 指标 | 基座 | s25 | s50 | s75 | s100 | s125 | s150 |
|---|---|---|---|---|---|---|---|
| **mini8 mean@1** | 0.5864 | 0.6390 | 0.6233 | 0.6869 | 0.7161 | 0.6668 | **0.6764** |
| **64 行训练内 val** | 0.4730 | 0.4863 | 0.4883 | 0.4951 | 0.4894 | 0.4914 | **0.5080（新高）** |

s150 vs 基座：4 胜 4 负 0 平（**+0.090，+15.4%**）。s150 vs s100（mini8 峰值）：1 胜 4 负 3 平。

## E.2 解读

- **终版在两套指标上的定性**：64 行 val（64 倍样本）以 **0.5080 收官创新高**——训练全程单调上行的最终确认；mini8 落在平台期噪声带内（0.6764 vs 峰值 0.7161，单样本权重 0.125）。
- **最难样本 p4（"Lost City Near"）在 step-150 首次攻破**：0.000→**0.667**（此前 5 个 ckpt 最高仅 0.200）——长文本渲染能力在训练后段仍在实质性进步。
- p7 再度回落（0.822→0.389）：该样本全程在 0.39-0.94 间宽幅震荡，属未收敛的单一样本，不影响整体趋势。
- **最终结论**：150 步 DanceGRPO 使视频文字渲染 OCR 指标相对基座 +15.4%（mini8）/ +7.4%（64 行 val，0.4730→0.5080），难例零分样本清零，短文本样本全部满分或近满分。1018 行全量评测（基线 + 终版）进行中，将以三倍量级数据给出最终裁决。

## E.3 产物

- 视频 `workspace/val_generations/eval_step150/150/{0..7}.mp4`；明细 `eval_step150/150.jsonl`；日志 `logs/eval_step150.log`。
- 训练收官日志 `logs/formal_run.log`（三段拼接：0-42 / 25-50 / 50-150）。
